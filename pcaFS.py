...
def pca_feature_correlation(X, X_pca, explained_var, features:list=None, fig_dpi:int=150, save_plot:bool=False):
    ...
    # Normalize & Sort
    df_n, df_na, df_nabv = normalize_dataframe(df_dotproduct, explained_var, plot_opt=False, save_plot=save_plot)
...
if __name__ == '__main__':
    """ IRIS """
    data = pd.read_hdf("NewFeatures.h5", key = 'features')
    #data = data.drop(columns="target")
    featureNames = list(data.columns)
    outputs = pca_full_report(X=data.to_numpy(), features_=featureNames, save_plot=True)